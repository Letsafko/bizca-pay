...
- "Find security issues in `UserService.cs`" → `security-auditor`
- "What tests are missing for the create-user flow?" → `test-engineer`
...
> *POST `/api/users` occasionally returns 500 instead of 201. It happens roughly once every 50 requests under load. No errors in structured logs. Started after last week's deployment.*

Plausible root causes (mutually exclusive, all fit the symptoms):

1. A race condition in the `User.Create` → `SaveChangesAsync` flow under concurrent requests (missing unique index on `email`)
2. An `IDateTimeProvider` implementation that occasionally returns a `default` value, causing an EF Core constraint violation
3. A connection pool exhaustion when Testcontainers-based tests run in parallel alongside the service
4. A Npgsql transient error that is swallowed by a missing `retry` policy in the `HttpClient` / infrastructure layer
...
three teammates using the existing agent types:
  ...
  - code-reviewer  — investigate race conditions, missing unique indexes,
                     and blocking/async issues in the create-user code path
  - security-auditor — investigate auth checks, input validation at the API
                       boundary, and any missing idempotency guards
  - test-engineer  — propose tests (unit + Testcontainers integration) that
                     would distinguish between the hypotheses and check
                     coverage gaps in the create-user flow
...
4. When `code-reviewer` finds that `SaveChangesAsync` is called without a unique index on `email`, it messages `security-auditor` to confirm there's no auth bypass risk in the duplicate-user path. `security-auditor` checks and replies — either confirming the race is the real issue or producing counter-evidence.
5. `test-engineer` proposes a focused Testcontainers integration test (concurrent `POST /api/users` with the same email) for whichever theory is winning, which the team uses to verify before declaring consensus.
