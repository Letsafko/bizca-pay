...
3. **`test-engineer`** — Analyze test coverage for the change. Identify gaps in happy path, `Result<T>` failure paths, edge cases, and HTTP status mapping. Output the standard coverage analysis.
...
1. **Code Quality** — Aggregate Critical/Important findings from `code-reviewer` and any failing tests or build output. Resolve duplicates between reviewers.
...
3. **Performance** — Pull EF Core / async findings from `code-reviewer`'s performance axis.
4. **Infrastructure** — Verify directly:
   - EF Core migrations applied: `dotnet ef database update`
   - No secrets committed (`appsettings.Local.json` in `.gitignore`, User Secrets / env vars for CI)
   - Health check endpoint responds (`/health` via `AddHealthChecks().AddNpgSql(...)`)
   - `dotnet list package --vulnerable` — zero results
   - Feature flags / environment-specific config correct
5. **Documentation** — README, ADRs, XML doc comments on public API types, CHANGELOG updated.
